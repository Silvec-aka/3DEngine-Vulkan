...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  ...
  setupDebugMessenger(); // debug messenger to catch validation layer messages
  createSurface(); // connection between vulkan and window display
  pickPhysicalDevice(); // choose the physical device we will use
  createLogicalDevice(); // create logical device to interface with physical device
  createCommandPool(); // create command pool to send commands to the device
}
...
void LveDevice::pickPhysicalDevice() {
  ...
  // take the first suitable device
